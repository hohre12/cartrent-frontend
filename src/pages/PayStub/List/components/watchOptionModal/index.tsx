...
const WatchOptionModal = (props: TModal) => {
  ...
  const [selectedHideWatchOptions, setSelectedHideWatchOptions] =
    useRecoilState(selectedPayStubHideWatchOptionsState);

  const hideWatchOptionKeys = useMemo(() => {
    return Object.keys(
      PAYSTUB_LIST_WATCH_OPTIONS,
    ) as (keyof typeof PAYSTUB_LIST_WATCH_OPTIONS)[];
  }, []);

  const handleSetWatchOptions = useCallback(
    (key: string) => {
      if (!PAYSTUB_LIST_WATCH_REQUIRED_OPTIONS.includes(key)) {
  ...
  const handleSetAllWatchOptions = useCallback(() => {
    if (
      ...
      hideWatchOptionKeys.filter(
        (it) => !PAYSTUB_LIST_WATCH_REQUIRED_OPTIONS.includes(it),
      ).length
    ) {
        ...
        <div className="optionWrapper">
          {hideWatchOptionKeys.map((it, idx) => (
            <Button
              ...
              size="large"
              className={[
                `${selectedHideOptionsList.includes(it) ? '' : 'active'} ${PAYSTUB_LIST_WATCH_REQUIRED_OPTIONS.includes(it) ? 'required' : ''}`,
              ].join(' ')}
              onClick={() => handleSetWatchOptions(it)}
            >
              <div>{PAYSTUB_LIST_WATCH_OPTIONS[it]}</div>
              {PAYSTUB_LIST_WATCH_REQUIRED_OPTIONS.includes(it) ? (
