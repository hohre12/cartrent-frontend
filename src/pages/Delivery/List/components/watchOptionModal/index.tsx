...
const WatchOptionModal = (props: TModal) => {
  ...
  const [selectedHideWatchOptions, setSelectedHideWatchOptions] =
    useRecoilState(selectedDeliveryHideWatchOptionsState);

  const hideWatchOptionKeys = useMemo(() => {
    return Object.keys(
      DELIVERY_LIST_WATCH_OPTIONS,
    ) as (keyof typeof DELIVERY_LIST_WATCH_OPTIONS)[];
  }, []);

  const handleSetWatchOptions = useCallback(
    (key: string) => {
      if (!DELIVERY_LIST_WATCH_REQUIRED_OPTIONS.includes(key)) {
  ...
  const handleSetAllWatchOptions = useCallback(() => {
    if (
      ...
      hideWatchOptionKeys.filter(
        (it) => !DELIVERY_LIST_WATCH_REQUIRED_OPTIONS.includes(it),
      ).length
    ) {
        ...
        <div className="optionWrapper">
          {hideWatchOptionKeys.map((it, idx) => (
            <Button
              ...
              size="large"
              className={[
                `${selectedHideOptionsList.includes(it) ? '' : 'active'} ${DELIVERY_LIST_WATCH_REQUIRED_OPTIONS.includes(it) ? 'required' : ''}`,
              ].join(' ')}
              onClick={() => handleSetWatchOptions(it)}
            >
              <div>{DELIVERY_LIST_WATCH_OPTIONS[it]}</div>
              {DELIVERY_LIST_WATCH_REQUIRED_OPTIONS.includes(it) ? (
