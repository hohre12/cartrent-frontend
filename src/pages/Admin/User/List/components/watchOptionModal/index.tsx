...
const WatchOptionModal = (props: TModal) => {
  ...
  const [selectedHideWatchOptions, setSelectedHideWatchOptions] =
    useRecoilState(selectedUserHideWatchOptionsState);

  const hideWatchOptionKeys = useMemo(() => {
    return Object.keys(
      USER_LIST_WATCH_OPTIONS,
    ) as (keyof typeof USER_LIST_WATCH_OPTIONS)[];
  }, []);

  const handleSetWatchOptions = useCallback(
    (key: string) => {
      if (!USER_LIST_WATCH_REQUIRED_OPTIONS.includes(key)) {
  ...
  const handleSetAllWatchOptions = useCallback(() => {
    if (
      ...
      hideWatchOptionKeys.filter(
        (it) => !USER_LIST_WATCH_REQUIRED_OPTIONS.includes(it),
      ).length
    ) {
        ...
        <div className="optionWrapper">
          {hideWatchOptionKeys.map((it, idx) => (
            <Button
              ...
              size="large"
              className={[
                `${selectedHideOptionsList.includes(it) ? '' : 'active'} ${USER_LIST_WATCH_REQUIRED_OPTIONS.includes(it) ? 'required' : ''}`,
              ].join(' ')}
              onClick={() => handleSetWatchOptions(it)}
            >
              <div>{USER_LIST_WATCH_OPTIONS[it]}</div>
              {USER_LIST_WATCH_REQUIRED_OPTIONS.includes(it) ? (
