...
const WatchOptionModal = (props: TModal) => {
  ...
  const [selectedHideWatchOptions, setSelectedHideWatchOptions] =
    useRecoilState(selectedAgencyContractHideWatchOptionsState);

  const hideWatchOptionKeys = useMemo(() => {
    return Object.keys(
      AGENCY_CONTRACT_LIST_WATCH_OPTIONS,
    ) as (keyof typeof AGENCY_CONTRACT_LIST_WATCH_OPTIONS)[];
  }, []);

  const handleSetWatchOptions = useCallback(
    (key: string) => {
      if (!AGENCY_CONTRACT_LIST_WATCH_REQUIRED_OPTIONS.includes(key)) {
  ...
  const handleSetAllWatchOptions = useCallback(() => {
    if (
      ...
      hideWatchOptionKeys.filter(
        (it) => !AGENCY_CONTRACT_LIST_WATCH_REQUIRED_OPTIONS.includes(it),
      ).length
    ) {
        ...
        <div className="optionWrapper">
          {hideWatchOptionKeys.map((it, idx) => (
            <Button
              ...
              size="large"
              className={[
                `${selectedHideOptionsList.includes(it) ? '' : 'active'} ${AGENCY_CONTRACT_LIST_WATCH_REQUIRED_OPTIONS.includes(it) ? 'required' : ''}`,
              ].join(' ')}
              onClick={() => handleSetWatchOptions(it)}
            >
              <div>{AGENCY_CONTRACT_LIST_WATCH_OPTIONS[it]}</div>
              {AGENCY_CONTRACT_LIST_WATCH_REQUIRED_OPTIONS.includes(it) ? (
